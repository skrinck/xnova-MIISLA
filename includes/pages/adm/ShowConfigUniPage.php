...
function ShowConfigUniPage()
{
	...
	if (!empty($_POST))
	{
		$config_before = array(
			...
			'antimatter_start'		=> $config->antimatter_start,
			...
			'ally_date'   => $config->ally_date * (3600 * 24),
		);
		
		$game_disable			= isset($_POST['closed']) && $_POST['closed'] == 'on' ? 1 : 0;
		$comments			= isset($_POST['comments']) && $_POST['comments'] == 'on' ? 1 : 0;
		...
		$antimatter_start		= HTTP::_GP('antimatter_start', 0);
		...
		$ally_date				= HTTP::_GP('ally_date', 0) * (3600 * 24);
		...
		$config_after = array(
			...
			'antimatter_start'		=> $antimatter_start,
	...
	$template->assign_vars(array(
		...
		'se_antimatter_start'			=> $LNG['se_antimatter_start'],
		'se_antimatter_start_info'		=> $LNG['se_antimatter_start_info'],
		...
		'se_server_op_comments' => $LNG['se_server_op_comments'],
		'se_ally_date'					=> $LNG['se_ally_date'],
		'se_ally_date_description'					=> $LNG['se_ally_date_description'],
		'game_name'						=> $config->game_name,
		'uni_name'						=> $config->uni_name,
