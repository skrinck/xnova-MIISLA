...
function ShowQuickEditorPage()
{
	...
	switch($edit)
	{
		...
		case 'player':
			...
			$UserData	= $GLOBALS['DATABASE']->getFirstRow("SELECT ".$SpecifyItemsPQ." `username`, `authlevel`, `galaxy`, `system`, `planet`, `id_planet`, `darkmatter`, `authattack`, `authlevel`, `isChat`,`isMP` FROM ".USERS." WHERE `id` = '".$id."';");
...
//				$SQL	.= "`antimatter` = '".max(HTTP::_GP('antimatter', 0), 0)."', ";
				$SQL	.= "`isChat` = '".max(HTTP::_GP('isChat', 0), 0)."', ";
				$SQL	.= "`isMP` = '".max(HTTP::_GP('isMP', 0), 0)."', ";
...
//				'antimatter'	=> floattostring($UserData['antimatter']),
//				'antimatter_c'	=> pretty_number($UserData['antimatter']),
				'isChat'	=> floattostring($UserData['isChat']),
				'isMP'	=> floattostring($UserData['isMP']),
				'isMP'	=> pretty_number($UserData['isMP']),
				'isChat'	=> pretty_number($UserData['isChat']),
			));
