...
function ShowCreatecampaignPage()
{
	...
	if (!empty($_POST))
	{
		...
		if(!in_array($special_donation_status, $special_donation_valid)){
			$showMsg[]	= "Existen solo 2 opciones, para bonos de compras extra: Activado & Desactivado";
		}elseif($special_donation_academy > 100){
			$showMsg[]	= "El valor máximo permitido para la reducción de puntos de la  academia es 100%";
		}elseif($special_donation_stardust > 100){
			$showMsg[]	= "El valor máximo permitido para la reducción de mineral estelar es 100%";
		}elseif($darkmatter_reduc > 100){
			$showMsg[]	= "El valor máximo permitido para la reducción del costo de la materia oscura es 100%";
		}elseif($collider_promo > 100){
			$showMsg[]	= "El valor máximo permitido para la reducción del precio de recolección es 100%";
		}elseif(!in_array($primebuild, $primebuild_valid)){
			$showMsg[]	= "Existen solo 2 opciones para la opción de premio por construcción: Activado & Desactivado";
		}elseif(!in_array($auctionExpe, $auctionExpe_valid)){
			$showMsg[]	= "Existen solo 2 opciones para la opción de evento supremo: Si & No";
		}elseif($starDate > $endDate || $starDate == $endDate){
			$showMsg[]	= "La fecha de inicio debe ser antes que la fecha de culminación de la campaña";
	...
	$template->loadscript('styles/assets/js/core/libraries/jquery_ui/core.min.js');
	$template->loadscript('styles/assets/js/plugins/forms/wizards/form_wizard/form.min.js');
	$template->loadscript('styles/assets/js/plugins/forms/wizards/form_wizard/form_wizard.min.js');
	...
	$template->loadscript('styles/assets/js/core/libraries/jasny_bootstrap.min.js');
	$template->loadscript('styles/assets/js/plugins/forms/validation/validate.min.js');
	...
	$template->loadscript('styles/assets/js/pages/wizard_form.js');
