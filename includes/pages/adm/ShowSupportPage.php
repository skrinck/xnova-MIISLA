...
class ShowSupportPage
{
	...
	function __construct() 
	{
		...
		$this->tplObj->assign_vars(array('categoriaT'=>$cat));
	...
	public function show()
	{
		...
		while($ticketRow = $GLOBALS['DATABASE']->fetch_array($ticketResult)) {
			...
			$ticketRow['LastMessage']	= substr($LastMessage['message'], 0, 100).' ...';
		...
		$this->tplObj->loadscript('styles/assets/js/core/libraries/jquery_ui/datepicker.min.js');	
		$this->tplObj->loadscript('styles/assets/js/plugins/tables/datatables/datatables.min.js');	
		$this->tplObj->loadscript('styles/assets/js/plugins/tables/datatables/extensions/natural_sort.js');	
		$this->tplObj->loadscript('styles/assets/js/plugins/forms/selects/select2.min.js');	
		$this->tplObj->loadscript('styles/assets/js/core/app.js');	
		$this->tplObj->loadscript('styles/assets/js/pages/tasks_list.js');	
	...
	function changestatus() 
	{
		...
		if($action == "close"){
			...
			$message = $Friend_LNG['fl_ticket_auto_4'];
			$GLOBALS['DATABASE']->multi_query("INSERT INTO ".TICKETS_ANSWER." SET ticketID = ".$ticketID.", ownerID = ".$USER['id'].", ownerName = '".$USER['username']."', subject = '".$GLOBALS['DATABASE']->sql_escape($subject)."', message = '".$GLOBALS['DATABASE']->sql_escape($message)."', time = ".TIMESTAMP.";UPDATE ".TICKETS." SET status = 99 WHERE ticketID = ".$ticketID.";");
...
/* 		if($change && $status == 1) {
	...
	function view() 
	{
		...
		$latestMessages = $GLOBALS['DATABASE']->query("SELECT * FROM ".TICKETS_ANSWER." ORDER BY time DESC LIMIT 5;");
		$todos=[];
		while($answerRow = $GLOBALS['DATABASE']->fetch_array($latestMessages)) {
			$answerRow['timem']=pretty_time(TIMESTAMP - $answerRow['time']);
			$todos[]=$answerRow;
		}
		...
		$this->tplObj->loadscript('styles/assets/js/core/app.js');	
		$this->tplObj->loadscript('styles/assets/js/pages/support_chat_layouts.js');	
