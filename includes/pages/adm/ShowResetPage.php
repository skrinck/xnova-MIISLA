...
function ShowResetPage()
{
	...
	{		
		...
		$resourceUserD = array(921);
		foreach($resourceUserD as $ID)
		{
			if(isset($config->{$resource[$ID].'_start'}))
			{
				$dbcol['resource_user_start_darkmatter'][$ID]	= "`".$resource[$ID]."` = ".$config->{$resource[$ID].'_start'};
			}
		}
		$resourceUserA = array(922);
		foreach($resourceUserA as $ID)
		{
			if(isset($config->{$resource[$ID].'_start'}))
			{
				$dbcol['resource_user_start_antimatter'][$ID]	= "`".$resource[$ID]."` = ".$config->{$resource[$ID].'_start'};
			}
		}
	...
	$GLOBALS['DATABASE']->multi_query("DELETE FROM ".USERS." WHERE `universe` = ".Universe::getEmulated()." AND `id` != '".$ID."';DELETE FROM ".PLANETS." WHERE `universe` = ".Universe::getEmulated()." AND `id_owner` != '".$ID."';");
		...
		// RECURSOS
		if ($_POST['dark']	==	'on'){
			$GLOBALS['DATABASE']->query("UPDATE ".USERS." SET ".implode(", ",$dbcol['resource_user_start_darkmatter'])." WHERE `universe` = ".Universe::getEmulated().";");
		}

		if ($_POST['antimatter']	==	'on'){
			$GLOBALS['DATABASE']->query("UPDATE ".USERS." SET ".implode(", ",$dbcol['resource_user_start_antimatter'])." WHERE `universe` = ".Universe::getEmulated().";");
		}
		...
		if ($_POST['rw']	==	'on'){
			...
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".RW.";");
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".TOPKB_USERS.";");
		}


		if ($_POST['friends']	==	'on'){
			$GLOBALS['DATABASE']->query("DELETE FROM ".BUDDY." WHERE `universe` = ".Universe::getEmulated().";");
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".BUDDY_REQUEST.";");
		}

		if ($_POST['alliances']	==	'on'){
			$GLOBALS['DATABASE']->multi_query("DELETE FROM ".ALLIANCE." WHERE `ally_universe` = '".Universe::getEmulated()."' AND id != 1;UPDATE ".USERS." SET `ally_id` = '0', `ally_register_time` = '0', `ally_rank_id` = '0' WHERE `universe` = ".Universe::getEmulated()." AND ally_id != 1;");
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".ALLIANCE_RANK.";");
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".ALLIANCE_REQUEST.";");
		}
		...
		if ($_POST['comments']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".COMMENTSHOF.";");
		}

		//Admin Logins
		if ($_POST['admlogins']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".ADMINLOGINS.";");
		}

		//bots
		if ($_POST['bots']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".BOTS.";");
		}

		//Notifications
		if ($_POST['notifications']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".NOTIF.";");
		}

		//Messages Chat
		if ($_POST['mchat']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".CHAT_MES.";");
		}

		//CRONJOBS_LOG
		if ($_POST['cronjoblogs']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".CRONJOBS_LOG.";");
		}

		//ACS- SAC
		if ($_POST['acs']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".USERS_ACS.";");
		}

		//TICKETS_ANSWER
		if ($_POST['tickets']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".TICKETS.";");
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".TICKETS_ANSWER.";");
		}

		//re_reset_tracking
		if ($_POST['tracking']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".TRACKING.";");
		}

		//fleet logs
		if ($_POST['fleetlogs']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".LOG_FLEETS.";");
		}

		//saved coord
		if ($_POST['savedcoord']	==	'on'){
			$GLOBALS['DATABASE']->query("TRUNCATE TABLE ".SAVEDGAL.";");
		}
	...
	$template->assign_vars(array(	
		...
		're_resources_antimatter'			=> $LNG['re_resources_antimatter'],
		...
		're_reset_comments'					=> $LNG['re_reset_comments'],
		're_reset_adm_logins'				=> $LNG['re_reset_adm_logins'],
		're_reset_bots'						=> $LNG['re_reset_bots'],
		're_reset_notifications'			=> $LNG['re_reset_notifications'],
		're_reset_messages_chat'			=> $LNG['re_reset_messages_chat'],
		're_reset_cronjob'					=> $LNG['re_reset_cronjob'],
		're_reset_acs'						=> $LNG['re_reset_acs'],
		're_reset_tickets'					=> $LNG['re_reset_tickets'],
		're_reset_tracking'					=> $LNG['re_reset_tracking'],
		're_reset_fleet_logs'				=> $LNG['re_reset_fleet_logs'],
		're_reset_savedcoord'				=> $LNG['re_reset_savedcoord'],
	));
