...
function ShowAccountDataPage()
{
	...
	$template->loadscript('styles/assets/js/plugins/notifications/bootbox.min.js');	
	$template->loadscript('styles/assets/js/plugins/notifications/sweet_alert.min.js');		
	$template->loadscript('styles/assets/js/core/app.js');
	$template->loadscript('styles/assets/js/pages/components_modals.js');
	...
	if (!empty($id_u))
	{
		...
		else
		{
			...
			$SpecifyItemsU	= 
			"u.id,u.username,u.email,u.email_2,u.authlevel,u.usertext,u.id_planet,u.galaxy,u.system,u.planet,u.user_lastip,u.ip_at_reg,u.darkmatter,u.register_time,u.onlinetime,u.urlaubs_modus,u.
			...
			while ($PlanetsWhile	= $GLOBALS['DATABASE']->fetch_array($PlanetsQuery))
			{
				...
				{	
					...
					<tr>
						...
						<td class='text-center'><a title='".$LNG['ad_pla_edit_fields']."' href=\"javascript:openEdit(".$PlanetsWhile['id'].",'planet')\"><i class='fa fa-edit'></i></a></td>
						</tr>";
			...
			$visitList	= array();
			$QuerySearch	= $GLOBALS['DATABASE']->query("SELECT * FROM ".TRACKING." WHERE trackMode = 1 AND userId = ".$GLOBALS['DATABASE']->sql_escape($id_u)." ORDER BY time DESC LIMIT 100;");
			...
			$result    = $GLOBALS['DATABASE']->query("SELECT * FROM ".TRACKING." WHERE trackMode != 1 AND userId = ".$GLOBALS['DATABASE']->sql_escape($id_u)." ORDER BY TIME DESC LIMIT 100");
			...
			$ramesult    = $GLOBALS['DATABASE']->query("SELECT * FROM ".NOTES." WHERE owner = ".$GLOBALS['DATABASE']->sql_escape($id_u));
            $notes    =    array();
            foreach ($ramesult as $row)
            {           
                $row['timem']=pretty_time(TIMESTAMP - $row['time']);
                $notes[]=$row;
            }
            $GLOBALS['DATABASE']->free_result($ramesult);

            $result    = $GLOBALS['DATABASE']->query("SELECT a.sender, a.id as buddyid, b.id, b.username, b.onlinetime, b.galaxy, b.system, b.planet, b.ally_id, c.ally_tag, d.text
            FROM (".BUDDY." as a, ".USERS." as b) LEFT JOIN ".ALLIANCE." as c ON c.id = b.ally_id LEFT JOIN ".BUDDY_REQUEST." as d ON a.id = d.id
            WHERE (a.sender = ".$GLOBALS['DATABASE']->sql_escape($id_u)." AND a.owner = b.id) OR (a.owner = ".$GLOBALS['DATABASE']->sql_escape($id_u)." AND a.sender = b.id );");
            $buddys    =    array();
