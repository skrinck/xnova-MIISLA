...
function ShowSendMessagesPage() {
	...
	if ($ACTION == 'send')
	{
		...
		if($typemsg == 0){
			...
			$pmSubject 	= ''.$Subject.'';
			$pmMessage 	= ''.BBCode::parse($Message).'';

			$USERS		= $GLOBALS['DATABASE']->query("SELECT `id`, `username`, `foto` FROM ".USERS." WHERE `universe` = 1 AND onlinetime > ".(TIMESTAMP > 7 * 24 * 60 * 60).";");
			foreach($USERS as $UserData)
			{				
				$sendMessage = str_replace('{USERNAME}', $UserData['username'], $pmMessage);
				...
				$SQL     .= "`noText` = '".$From." » <b>".$pmSubject.":</b> <br>".$sendMessage."', ";
				$SQL     .= "`noImage` = '".$USER['foto']."', ";
				$SQL     .= "`isType` = 99;";
				$GLOBALS['DATABASE']->query($SQL);
			}
		}elseif($typemsg == 2){
			require_once 'includes/classes/BBCode.class.php';
			require_once 'includes/libs/pusher/Pusher.php';
			$deviceId = "";
