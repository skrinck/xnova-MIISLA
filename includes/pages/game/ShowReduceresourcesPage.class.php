...
{
	public static $requireModule = MODULE_RDUCE_RESOURSE;
//	public static $requireModule = 0;
	...
	public function reduce()
	{
		...
		if (!isset($_POST['palanets'])){
			...
			$EndLog			= array();
			$TestCalcRoom   = array(203 => 1);
			$FleetRoomDark 	= FleetFunctions::GetFleetRoom($TestCalcRoom, $USER);	
			foreach($_POST['palanets'] as $ID => $Value) {
				$sql	= 'SELECT id, id_owner, name, big_ship_cargo, galaxy, system, planet, planet_type, metal, crystal, deuterium FROM %%PLANETS%% WHERE id = :Value AND id_owner = :userId;';
				...
				$needed_ships 				= min($needed_ships, $sur['big_ship_cargo']);
				$rawfleetarray				= array(203 => $needed_ships);
				...
				$consumption		= FleetFunctions::GetFleetConsumption($rawfleetarray, $Duration, $Distance, $USER, $SpeedFactor);
...
//			$this->printMessage("This page will be again available very soon", true, array('game.php?page=overview', 2));
		...
		foreach($planetsResult as $planetRow) {				
			list($USER, $planetRow)		= $planetUpdater->CalcResource($USER, $planetRow, true, TIMESTAMP);
				...
				$SpeedFactor    	= FleetFunctions::GetGameSpeedFactor();
				$MaxFleetSpeed		= FleetFunctions::GetFleetMaxSpeed(203, $USER);
			...
			$PlanetListin[$planetRow['id']] = array(
				...
				'ev_transporter'		=> $planetRow['big_ship_cargo'],
