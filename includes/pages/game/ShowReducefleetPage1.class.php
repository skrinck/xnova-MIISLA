...
{
	public static $requireModule = MODULE_RDUCE_FLEET;
	...
	public function reduce()
	{
		...
		if($PLANET['gal6mod'] == 1){
			...
			foreach($reslist['fleet'] as $Item){
				$select_items		.= $resource[$Item].",";
				//$select_items				= $PLANET[$resource[$Item]];
			}	
			
			foreach($_POST['palanets'] as $ID => $Value) {
				$FleetStatCount++;
				$db	= Database::get();
				$sql	= 'SELECT '.$select_items.' id, id_owner, name, image, galaxy, system, planet, planet_type, metal, crystal, deuterium FROM %%PLANETS%% WHERE id = :Value;';
				...
				$elementPlanet	= array_merge(array(), $elementPlanetBattle, $elementPlanetTransport, $elementPlanetProcessorcs, $elementPlanetSpecial);

				echo json_encode($elementPlanet);exit;
				...
				$duration      				= FleetFunctions::GetMissionDuration($speed, $MaxFleetSpeed, $distance, $GameSpeedFactor, $USER);	
