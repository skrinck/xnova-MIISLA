		...
		if (count($CurrentQueue) == 0) {
			...
			foreach($CurrentQueue as $ListIDArray)
			{
				...
				if($ListIDArray[4] != $PLANET['id']) {
					//FIxed cancelar investigaciones de otra colonia
	...
	private function AddBuildingToQueue($elementId, $AddMode = true)
	{
		...
		if($ActualCount == 0)
		{
			...
			$account_after = array(
				...
				'research'				=> $LNG['tech'][$elementId]." ".$BuildLevel,
			);
			
			$LOG = new Logcheck(26);
			$LOG->username = $USER['username'];
			$LOG->pageLog = "page=research [".$PLANET['galaxy'].":".$PLANET['system'].":".$PLANET['planet']."] [Investigar]";
