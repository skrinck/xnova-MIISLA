...
class GalaxyRows
{
	...
	public function getGalaxyData()
	{
		...
		p.galaxy, p.system, p.planet, p.id, p.id_owner, p.gal6mod, p.name, p.image, p.last_update, p.diameter, p.temp_min, p.destruyed, p.der_metal, p.der_crystal, p.der_deuterium, p.id_luna, 
		u.id as userid, u.ally_id, u.username, u.authlevel, u.onlinetime, u.urlaubs_modus, u.banaday, 
		...
		a.id as allyid, a.ally_tag, a.ally_web, a.ally_members, a.ally_name, (a.ally_owner=u.id) as is_leader, a.ally_owner_range, 
	...
	protected function getAllowedMissions()
	{
		...
		$this->galaxyData[$this->galaxyRow['planet']]['missions']	= array(
			...
			8	=> isModuleAvailable(MODULE_MISSION_RECYCLE) && $PLANET[$resource[209]] || $PLANET[$resource[219]],
			9	=> !$this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $PLANET[$resource[214]] > 0 && isModuleAvailable(MODULE_MISSION_DESTROY) && $USER['rpg_destructeur'] > 0,
			...
			16	=> $this->galaxyRow['id_owner'] == NULL && $this->galaxyRow['gal6mod'] == 0 && isModuleAvailable(MODULE_MISSION_RECYCLE) && $PLANET[$resource[209]] || $PLANET[$resource[219]],
		);
	}
...
exit;*/
        ...
        $this->galaxyData[$this->galaxyRow['planet']]['user']	= array(
			...
			'allyname'    			=> $this->galaxyRow['ally_name'],
            'members'   			=> $this->galaxyRow['ally_members'],
		);
	}
