...
class MissionCaseSpy extends MissionFunctions implements Mission
{
	...
	function TargetEvent()
	{
		...
		/*$sql	= 'SELECT * FROM %%FLEETS%% WHERE fleet_end_id = :planetId AND fleet_mission = 5 AND fleet_end_stay > :time AND fleet_start_time < :time2;';

		$targetStayFleets	= $db->select($sql, array(
			':planetId'	=> $this->_fleet['fleet_end_id'],
			':time'		=> $this->_fleet['fleet_start_time'],
			':time2'	=> $this->_fleet['fleet_start_time'],
		));*/

		$sql	= 'SELECT * FROM %%FLEETS%% WHERE fleet_end_id 		= :planetId
		AND fleet_mission 		= 5
		...
		$SpyOffi		= $fleetAmount >= $MinAmount + 12 * SPY_VIEW_FACTOR;
		//$SpyArs			= $fleetAmount >= $MinAmount + 20 * SPY_VIEW_FACTOR;
		...
		if($SpyOffi) 
		{
			$classIDs[600]	= $reslist['officier'];
		}

		//if($SpyArs)
		//{
		//	$classIDs[1000]	= $reslist['ars'];
	//	}
