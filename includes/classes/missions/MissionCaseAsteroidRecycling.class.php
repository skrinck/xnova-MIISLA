...
class MissionCaseAsteroidRecycling extends MissionFunctions implements Mission
{
	...
	{	
		...
		if(!empty($targetData['total']))
		{
			...
			//$AllCapacity		= $recyclerStorage;

			$totalStorage = $recyclerStorage + min(0, $otherFleetStorage - $incomingGoods);
			//$totalStorage = $recyclerStorage + min(0, $otherFleetStorage - $incomingGoods);

			//$totalStorage		= $recyclerStorage + min($otherFleetStorage, $incomingGoods/100*$targetUser['asteroid_mine_tech']);
				//$AllCapacity	-= $totalStorage;
			...
			$sql	= "DELETE FROM %%PLANETS%% WHERE galaxy = :fleet_end_galaxy AND system = :fleet_end_system AND planet = :fleet_end_planet AND planet_type = :fleet_end_type; AND gal6mod = 1";
			...
			$sql	= "UPDATE %%USERS%% SET asteroid = asteroid + :asteroid WHERE id = :userId;";
		...
		$Message 	= sprintf($LNG['sys_recy_gotten_a'], 
