	...
	function EndStayEvent()
	{
		...
		switch($GetEvent)
		{
			...
			case 2:
				...
				**/
				...
				if(is_bonus_active('mo')){
					...
					$max_rate = 100 - $bonus_data['procent']; 
				}

				$eventSize   = mt_rand(0, $max_rate);
                $Size       = 0;
				...
				if(10 < $eventSize) {
					...
					$Size		= mt_rand(501, 700);
					$Message	= $LNG['sys_expe_found_dm_2_'.mt_rand(1,3)];
				} elseif(0 == $eventSize) {
					$Size	 	= mt_rand(701, 3000);
					$Message	= $LNG['sys_expe_found_dm_3_'.mt_rand(1,2)];
				}


				if ($Size >0){
					tournement($this->_fleet['fleet_owner'], 1, $Size);//mas materia oscura
					tournement($this->_fleet['fleet_owner'], 5, 1);//mision exitosa
					$Achievements = achievementSuccesDaily($ownerUser, $this->_fleet['fleet_start_id']);		
					$Achievements = achievementSuccesVaria($ownerUser);
				}
				if($Size >700){
					tournement($this->_fleet['fleet_owner'], 7, $Size);//mas materia oscura
				}
			...
			case 3:
				...
				if ($Found >0){
					tournement($this->_fleet['fleet_owner'], 6, 1);//mas naves
					tournement($this->_fleet['fleet_owner'], 5, 1);//mision exitosa
					$Achievements = achievementSuccesDaily($ownerUser, $this->_fleet['fleet_start_id']);		
					$Achievements = achievementSuccesVaria($ownerUser);
				}
			...
			$messageHTML	= <<<HTML
				...
				if($combatResult['won'] == 'a'){
					...
					//}
					tournement($this->_fleet['fleet_owner'], 5, 1);//mision exitosa
					tournement($this->_fleet['fleet_owner'], 8, 1);//Aliens/Piratas
				}
			...
			case 5:
			$this->KillFleet();
				tournement($this->_fleet['fleet_owner'], 2, 1);//Agujeros Negros
			...
			$maxChance 	   = mt_rand(0,1000);
				$sql	= "UPDATE %%USERS%% SET achievement_point = achievement_point + :achievement_point WHERE id = :userId;";
				database::get()->update($sql, array(
					':userId'       => $this->_fleet['fleet_owner'],
					':achievement_point'       => $maxChance,
				));
				$Message	= 'Has ganado Puntos de Logros!</span>';
				tournement($this->_fleet['fleet_owner'], 5, 1);//mision exitosa
				$Achievements = achievementSuccesDaily($ownerUser, $this->_fleet['fleet_start_id']);		
				$Achievements = achievementSuccesVaria($ownerUser);
			break;
