...
class AsteroidCronjob implements CronjobTask
{
	...
	{		
		...
		if($config->asteroid_event < TIMESTAMP){
			...
			$sql	= "DELETE FROM %%PLANETS%% WHERE id_owner = :id_owner AND gal6mod = :gal6mod AND universe = :universe;";
			$db->delete($sql, array(
				':id_owner'	=> 1,
				':gal6mod'	=> 1,
				':universe'	=> 1
			...
			foreach($galaxy as $Element){
				...
				foreach($system as $System_Element){
					...
					if(count($cautare)==0){
						$MA = $config->asteroid_metal;
						$CA = $config->asteroid_crystal;
						$DA= $config->asteroid_deuterium;
						$metal_rand = rand(0,$MA);
						$crystal_rand = rand(0,$CA);
						$deuterium_rand= rand(0,$DA);
			...
			foreach($totalPremiums as $userInfo){
				...
				$Subject 	= '<span class="admin">Evento Asteroid</span>';				
