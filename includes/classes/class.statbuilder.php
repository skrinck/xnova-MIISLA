...
class statbuilder
{
	function __construct()
	{
		...
		$this->memory		= array(round(memory_get_usage() / 2024,1),round(memory_get_usage(1) / 2024,1));
		$this->time   		= TIMESTAMP;
	...
	private function SomeStatsInfos()
	{
		return array(
			...
			'memory_peak'		=> array(round(memory_get_peak_usage() / 2024,1), round(memory_get_peak_usage(1) / 2024,1)),
			'initial_memory'	=> $this->memory,
			'end_memory'		=> array(round(memory_get_usage() / 2024,1), round(memory_get_usage(1) / 2024,1)),
	...
	private function GetUsersInfosFromDB()
	{
		...
		foreach($reslist['officier'] as $Officier){
			$select_officier		.= " u.".$resource[$Officier].",";
		}
		...
		$Return['Planets']	= $database->select('SELECT SQL_BIG_RESULT DISTINCT '.$select_buildings.' p.id, p.universe, p.id_owner, u.authlevel, u.bana, u.username, u.urlaubs_modus FROM %%PLANETS%% as p LEFT JOIN %%USERS%% as u ON u.id = p.id_owner;');
		$Return['Users']	= $database->select('SELECT SQL_BIG_RESULT DISTINCT '.$selected_tech.$select_fleets.$select_defenses.$select_officier.' u.id, u.ally_id, u.authlevel, u.bana, u.universe, u.username, u.urlaubs_modus, s.tech_rank AS old_tech_rank, s.build_rank AS old_build_rank, s.defs_rank AS old_defs_rank, s.fleet_rank AS old_fleet_rank, s.total_rank AS old_total_rank FROM %%USERS%% as u LEFT JOIN %%STATPOINTS%% as s ON s.stat_type = 1 AND s.id_owner = u.id LEFT JOIN %%PLANETS%% as p ON u.id = p.id_owner GROUP BY s.id_owner, u.id, u.authlevel;');
	...
	private function GetOfficierPoints($USER) 
	{
		global $resource, $reslist, $pricelist;
		$OfficierCounts = 0;
		$OfficierPoints = 0;

		foreach($reslist['officier'] as $office) 
		{
			if($USER[$resource[$office]] == 0) continue;

            // Points = (All resources / PointsPerCost) * Factor * ( 2 * ( Factor ^ Level ) - Factor) + 1)
            // PointsPerCot == Config::get()->stat_settings
			$OfficierCounts		+= $USER[$resource[$office]];
            $OfficierPoints     +=
                ($pricelist[$office]['cost'][901] + $pricelist[$office]['cost'][902] + $pricelist[$office]['cost'][903])
                * $pricelist[$office]['factor']
                * (
                    2 * (
                        pow($pricelist[$office]['factor'], $USER[$resource[$office]]) - $pricelist[$office]['factor']
                    ) + 1
                );


            $this->setRecords($USER['id'], $office, $USER[$resource[$office]]);
		}
		
		return array('count' => $OfficierCounts, 'points' => ($OfficierPoints / Config::get()->stat_settings));
	}
	...
	final public function MakeStats()
	{
		...
		foreach($TotalData['Users'] as $UserData)
		{
			...
			$OfficierPoints		= $this->GetOfficierPoints($UserData);
			...
			$UserPoints[$UserData['id']]['officier']['count'] 	= $OfficierPoints['count'];
			$UserPoints[$UserData['id']]['officier']['points'] 	= $OfficierPoints['points'];
