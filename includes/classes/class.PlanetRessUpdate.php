...
class ResourceUpdate
{
	...
	public function CreateHash() {
		...
		$Hash[]	= $this->USER['dm_resource_level'];//gobernadores
		$Hash[]	= $this->USER['dm_energie_level'];//gobernadores
	...
	{
		if($this->PLANET['planet_type'] == 3 || $this->USER['urlaubs_modus'] == 1)
			return;
	...
	public function ReBuildCache()
	{
		...
		{	
			...
			$engineerbon = 5 * $this->USER['rpg_ingenieur'];
		...
		$basic_store_metal_2moons			= $temp[901]['max'] * $this->config->storage_multiplier * STORAGE_FACTOR * (1 + $this->USER['factor']['ResourceStorage']);
		$basic_store_crystal_2moons			= $temp[902]['max'] * $this->config->storage_multiplier * STORAGE_FACTOR * (1 + $this->USER['factor']['ResourceStorage']);
		$basic_store_deuterium_2moons		= $temp[903]['max'] * $this->config->storage_multiplier * STORAGE_FACTOR * (1 + $this->USER['factor']['ResourceStorage']);
		...
		if($this->PLANET['energy_used'] == 0) {
			...
			$basic_prod_metal_2moons = ($temp[901]['plus'] * (1 + 0.02 * $this->USER[$resource[131]]) * $prodLevel + $temp[901]['minus']) * $this->config->resource_multiplier;
			$basic_prod_crystal_2moons = ($temp[902]['plus'] * (1 + 0.02 * $this->USER[$resource[132]]) * $prodLevel + $temp[902]['minus']) * $this->config->resource_multiplier;
			$basic_prod_deuterium_2moons = ($temp[903]['plus'] * (1 + 0.02 * $this->USER[$resource[133]]) * $prodLevel + $temp[903]['minus']) * $this->config->resource_multiplier;
	...
	//by linkin separar defensa de naves
