										...
										<fieldset>
											...
											<div class="collapse in" id="Frame8">
												...
												<table class="table table-bordered table-hover datatable-highlight">
													<thead>
														<tr>
															<th>{$LNG['se_multi_users'][0]}</th>
															<th>{$LNG['se_multi_users'][1]}</th>
															<th>{$LNG['se_multi_users'][2]}</th>
															<th>{$LNG['se_multi_users'][3]}</th>
															<th>{$LNG['se_multi_users'][4]}</th>
															<th>{$LNG['se_multi_users'][5]}</th>
															<th>{$LNG['se_multi_users'][6]}</th>
															<th>{$LNG['se_multi_users'][7]}</th>
														</tr>
													</thead>
													<tbody>
															
													{foreach $MultiLoginList as $userId => $onlineRow} 
														<tr>
															<td>{$onlineRow.userId}</td>
															<td>{$onlineRow.nickname}</td>
															<td>{$onlineRow.ipadress}</td>
															<td>{$onlineRow.opsystem}</td>
															<td>{$onlineRow.isp}</td>
															<td>{if $onlineRow.proxies == 0}<span class="label label-success">NO</span>{else}<span class="label label-danger">SI</span>{/if}</td>
															<td>{$onlineRow.isValid}</td>
															<td>{$onlineRow.timestamp}</td>
														</tr>
													{/foreach}	
