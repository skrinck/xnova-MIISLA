...
{block name="content"}
<div id="ally_content" class="conteiner_ticket container" style="width: 67%">
 ...
 {if $answerRow@first}
    ...
    <input name="id" value="{$ticketID}" type="hidden"> 
        <div class="build_band ticket_bottom_band">
			...
			<select class="bottom_band_submit" name="action" onchange="showIt(this.options[this.selectedIndex].value)"><option value="0" selected disabled>- Acción -</option><option value="1">Enviar a admin</option><option value="3">Desbloquear Flotas</option><option value="4">Desbanear</option><option value="5">Añadir Recursos</option><option value="2">Cerrar</option></select>
