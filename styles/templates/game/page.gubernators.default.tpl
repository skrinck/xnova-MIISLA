{block name="title" prepend}{$LNG.lm_gubernators}{/block}
{block name="content"}
<div id="ally_content" class="conteiner container" >
    <div class="gray_stripe" style="padding-right:0;">
	       {$LNG.lm_gubernators}<a href="game.php?page=officier"><input class="right_flank btn btn-dark btn-xs" style="padding:5px" value="{$LNG.lm_officiers}" type="button"> </a>   
	       <div class="ach_head_right">
	            {$LNG.achiev_2}: <span>{$achievement_point_used|number} / {$achievement_points|number} <i class="fas fa-star"></i></span>
	       </div><!--/ach_head_right-->             
    </div> 
	<div id="gubernators_elements" class="gob_elements" style="padding-top:10px; padding-bottom:5px;">    	
            ...
            <div class="head">
                {$LNG.tech.{$ID}} <span>[{$Element.level}/{$Element.maxLevel}] {if $Element.level> 0}<a href="game.php?page=gubernators&mode=resetgouvernor&id={$ID}" class="tooltip go_reset_tree" data-tooltip-content="{$LNG.go_rpts}">⇓</a>{/if}
				<span class="pull-right">{if $Element.timeLeft > 0}<span class="text-warning" style="font-weight: bold;" id="time_{$ID}">-</span> <a href="game.php?page=gubernators&mode=resetgouvernortime&id={$ID}" class="tooltip go_reset_tree" data-tooltip-content="{$LNG.go_reset}">⇓</a>{/if}</span>
            </div>
